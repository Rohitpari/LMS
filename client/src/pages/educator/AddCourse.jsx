      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          <div className='flex flex-col gap-1 '>
            ...
            <input onChange={e => setCoursePrice(e.target.value)} value={coursePrice} type="number"
              placeholder='0' className='outline-none md:py-2.5 w-28 px-3 rounded border border-gray-500 ' required />
        ...
        <div className='flex flex-col gap-1'>
          ...
          <input onChange={e => setDiscount(e.target.value)} value={discount}
            ...
            className='putline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500' required
            type="number" />
        </div>

        {/* Adding Chappter and lecture */}
              ...
              <div className='flex justify-between
               items-center p-4 border '>
                <div className='flex items-center'> 
                  <img src={assets.dropdown_icon} onClick={()=>handleChapter('toggle', chapter.chapterId)} alt="" width={14}
                    className={`mr-2 cursor-pointer transition-all ${chapter.collapsed && "-rotate-90"}`} />
                  <span className='font-semibold'>{chapterIndex + 1}{chapter.chapterTitle}</span>
                </div>
                <span className='text-gray-500'>{chapter.chapterContent.length} Lectures</span>
                <img className='cursor-pointer' src={assets.cross_icon} onClick={()=>handleChapter('remove',chapter.chapterId)} alt="" />
              </div>
              {!chapter.collapsed && (
                <div className='p-4'>
            ...
            <div className='fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50'>
              <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
                ...
                <div className='flex gap-2 my-2'>
                  <p>Is Preview Free</p>
                  <input type="checkbox"
                    className='mt-1 block cr py-1 px-2'
